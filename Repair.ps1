...
param([ValidateSet('Diagnose','ResetOfficeCache','RepairOffice')][string]$Action='Diagnose',[string]$OutputPath=(Join-Path ([Environment]::GetFolderPath('Desktop')) 'Access_Forms_Reports_Repair'))
...
[ordered]@{Action=$Action;AccessRunning=[bool](Get-Process MSACCESS -ErrorAction SilentlyContinue);CacheExists=(Test-Path $cachePath)}|ConvertTo-Json|Set-Content (Join-Path $OutputPath "PreRepair_$stamp.json")
if($Action -eq 'Diagnose'){Log '[COMPLETE] Snapshot saved.';exit 0}
try{if($Action -eq 'ResetOfficeCache' -and $PSCmdlet.ShouldProcess($cachePath,'Back up and reset Office cache')){if(Get-Process MSACCESS -ErrorAction SilentlyContinue){throw 'Close Microsoft Access before resetting the cache.'};if(Test-Path $cachePath){$backup="$cachePath.backup-$stamp";Move-Item $cachePath $backup -Force;New-Item -ItemType Directory $cachePath -Force|Out-Null;Log "[BACKUP] $backup"}}
